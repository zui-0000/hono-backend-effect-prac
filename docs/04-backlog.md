...
| 領域             | 記事の推奨                     | この repo                     | 判定       |
| ---------------- | ------------------------------ | ----------------------------- | ---------- |
| エラー応答       | Problem Details（RFC 9457）    | 独自 `{ errorCode, message }` | **決着**   |
...
~~**ページネーションが最優先。**~~ **2026-08-14 に決着（実装しない）。**
判断と理由は [`API契約/ページネーションを実装するか.md`](API契約/ページネーションを実装するか.md)。
`listUsers` ごと実装しないと決めたため、「カーソル方式かページ番号方式か」は
**実装する側の判断**になり、決めても使われない。用意してあった
`schema/src/shared/pagination/` の型 4 つは削除した（参照 0 件、生成物にも 0 件で、
消しても `openapi.yaml` はバイト単位で不変だった）。

**これで ⑦ に期限のある行は無くなった。** 残る 5 領域はいずれも後から足せる形をしている。

~~**エラー応答は先に潰す前提がある。**~~ **2026-08-14 に決着（寄せない）。**
判断と理由は [`API契約/エラー応答をRFC9457に寄せるか.md`](API契約/エラー応答をRFC9457に寄せるか.md)。
決め手は `detail` が「原因を外に出さない」方針と衝突すること、
実利のある `instance` と media type は**外部の消費者が居て初めて価値が出る**こと、
そして「語彙だけ借りる」案（`errorCode` → `type`）が**改名部分の得ゼロ**だったこと。

なお「検証を先に」という前提自体も誤りだった。エラーボディは `errorBody()` 1 箇所でしか
作られないため、形を変えても drift のリスクは変わらない。順序の依存は無い。

副産物として**本物の問題が 1 つ**見つかっている。`errorCode` は
`<HTTP ステータス><連番>` なので識別子にステータスが埋まっており、ステータスを変えると
事由が同じままコードが動く。**先頭 3 桁は「作られたときのステータス」と定義し、以後動かさない**
ことにした（`error-code.ts` に明記済み）。
...
| ----------------------------------- | ------------------------------------------------ | ----------------------------------------- |
| `validateQuery`                     | **一度も実行されない**（意図的に残した汎用の口） | クエリ検証が丸ごと壊れていても気付けない  |
...
`validateQuery` は**永久に未検証になることが確定した**。`listUsers` を実装しないと決めたので
「実装時が初回実行になる」という以前の見込みは消えている。

**それでも消さない。意図的に残している。** このリポジトリのバックエンドは汎用の土台として
作っており、`request` の入力源を header / body / params / query の 4 つ揃えてあること自体が
設計だから。「いま使う経路が無い」は消す理由にならない。

引き換えに、API テストでは絶対に覆えない経路が 1 つ残る。埋めるなら**単体テスト**になり、
[単体テストの追加](#3-単体テストの追加) の対象に入る（`classifyDbFailure` と同じ事情 —
実経路から踏めないものは偽の入力を渡して直接叩く）。`c.req.query()` が繰り返しパラメータの
最初の 1 つしか返さない件（`validateQuery` の doc 参照）も、そのとき一緒に判断する。
...
### ~~`listUsers`（契約が未定義）~~ — 2026-08-14 に「実装しない」で決着

**このリポジトリでは実装しない。** 一覧取得そのものが学習対象になっていないため。
あわせてページネーションの型 4 つも削除した
（[`API契約/ページネーションを実装するか.md`](API契約/ページネーションを実装するか.md)）。

これで**実装の積み残しは空**になった。`user` と `auth` の全ユースケースが実装済み。
