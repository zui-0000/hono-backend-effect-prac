...
| 段の中身                                     | 例                                                   | 扱い             |
| -------------------------------------------- | ---------------------------------------------------- | ---------------- |
| 素のコンビネータだけ                         | `Effect.asVoid` / `Effect.orDie`                     | **素のまま**     |
| ラムダを含む（＝その場固有のロジックがある） | `Effect.map((rows) => Option.fromNullable(rows[0]))` | **名前を付ける** |
...
なお、この規則そのものを各関数の doc に書かない。5 箇所で同じ説明を繰り返すことになる。
関数の doc に書くのは**その関数固有の「なぜ」**だけ（`takeFirstRow` なら
「なぜ `restore*` と名乗らないか」）。
...
> **Effect 公式の推奨とは分かれている。** `@effect/tsgo` の `unnecessaryPipeChain`
> （Style カテゴリ）は「連鎖した pipe は 1 つにまとめよ」と言い、この規約に 25 件の
...
**判断の順序・実例・命名・一意性の守り方は
[`★ドメインサービス/`](★ドメインサービス/) にまとめてある。**
「集約か、ドメインサービスか、command か」で迷ったらそちらを読む。

---

## 認証は経路の入口、認可はユースケースの中

**認証 (authentication)** は「あなたは誰か」、**認可 (authorization)** は
「あなたは何をしてよいか」。問いが違うので置き場も違う。

|            | 認証                               | 認可                                 |
| ---------- | ---------------------------------- | ------------------------------------ |
| 判断材料   | 署名と鍵。HTTP の外から来る事実    | 対象・関係・状態。**業務が決める**   |
| 変わるとき | 方式を変えるとき（JWT→セッション） | **仕様が変わるとき**（管理者を足す） |
| 置き場     | `shared/presentation/handler/`     | ユースケース + `domain/services/`    |
| 宣言       | routes の `auth: true`             | コマンド／クエリの入力に `actor`     |

`verifyBearer` が `shared/` にあるのは全経路で同じことをするから。そして
**`shared/` は `contexts/` を参照できない**（`shared-not-to-contexts`）ので、
そこに認可を置くことは構造上できない。共有基盤にコンテキスト固有の業務ルールは入らない。

置き場に迷ったら 2 つ聞く。**API を変えずに規則が変わりうるか**（「本人だけ」→
「管理者も可」は HTTP を 1 ミリも変えずに起きる ＝ 業務ルール）。
**判定にデータの読み込みが要るか**（「下書き状態のものだけ」なら集約を読む必要があり、
middleware では原理的に書けない）。

> **ロールが入ったら話が変わる。** 認可は粒度で 2 段に分かれ、
> 粗粒度（「管理者だけがこの経路を叩ける」）は routes で宣言的に書いてよい。
> いまはロールが無いので粗粒度が空で、細粒度だけが存在している。
> 参考: [DDD×CQRS の認可設計](https://zenn.dev/135yshr/articles/60d7d006c0f38f)

### コマンドは「操作の許可」、クエリは「データの可視性」…にはしなかった

上の記事はクエリ側の認可を**引く範囲を絞る**形（WHERE に actor を足す）で書くことを
勧めており、一度そう実装した。0 件 → `Option.none` → 404 となり、存在も漏れない。

**それでもコマンドと同じ「照合して弾く」に統一した。** 絞る形だと認可の失敗が 404 になり、
「認可の失敗は対象の有無に関わらず 403」という規則から外れる。
規則の表現がコマンドとクエリで割れるより、規則が 1 つであることを優先した。

副産物として、クエリにも application の関数（`getUserQuery`）ができ、
controller が「DTO を組み立てて application へ渡す」形で揃った。
それまでクエリだけ controller がポートを context から引いていた非対称も消えている。

### 認可の失敗は 403。404 に畳まない

RFC 9110 §15.5.4 は「禁止されたリソースの存在を隠したいなら 404 を返してもよい」と
明記しており（GitHub がプライベートリポジトリでそうしている）、一度その形で実装した。

**それでも 403 に戻した。** 404 に畳むと「無かった」のか「見せてもらえなかった」のかを
クライアントが永久に区別できない。GitHub の docs 自身が「404 が返って存在するはずなら
認証を確認しろ」と案内しているのは、その DX の悪さの裏返しでもある。
引き換えに実在の有無は漏れるが、id は uuid v7 で 74 bit のランダム部を持つため
総当たりで洗うのは非現実的。

`ForbiddenError` は 403 になる**前に**判定するので、対象を読む前に落ちる。
DB を引かずに済み、応答時間から実在を推測される余地も減る。

404 が死んだわけではない。**自分を削除した直後、まだ有効なトークンで自分を引く**と
認可は通って 404 になる（実測で確認）。契約から 404 を消していないのはこのため。
